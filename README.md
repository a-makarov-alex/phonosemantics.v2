...
-----
        ...
        - Language встроится в процесс следующим образом:
        
        при чтении слова узнаем его язык
        вызываем метод на чтение языка, собираем для него базу фонем
        парсим слово, используя только фонемы языка
        инициализируем HashMap<Object, int>, заносим туда +1 вердикт о наличии в языке каждого из найденных фонотипов
        если в банке фонем языка нет искомой фонемы, выводим в консоль (?), какие фонемы (или фонотипы?) оказались в списке, а в языке не описаны.
        отдельно выводим фонемы, которые в языке описаны, а в списке не встречены
        в результате имеем мапу с набором "фонотип: количество списков с этим фонотипом". Это число (value) будет использоваться как делитель рпи расчете параметров "per Word", а в параметре "per Phonemes" будут учитываться только слова языков, где обнаружен фонотип
